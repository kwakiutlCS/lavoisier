$(function() {
	...
	for (var i = 0; i < 4; i++) {
		...
		if (t < 10)
		    t = " "+t;
		ctx.fillText(t, dx+(0.12+0.12*j)*w, dy+(0.195+0.2*i)*h);
	    }
	} 
	ctx.restore();
    };
	...
	for (var k in puzzle.reagents) {
	    atoms.push(new Molecule(dx+0.05+counter*0.15, dy+0.07, puzzle.reagents[k], 2))
	    counter += 1;
	}
	counter = 0;

	for (var k in puzzle.products) {
	    atoms.push(new Molecule(dx+0.63+counter*0.15, dy+0.07, puzzle.products[k], 2))
	    counter += 1;
	}
