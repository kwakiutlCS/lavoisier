$(function() {
	...
	for (var k in puzzle.reagents) {
		...
		tmp2[i] = puzzle.reagents[k][i];
	    }
	    atoms.push(new Molecule(0.05+counter*0.15, 0.07,tmp, 2))
	    atoms.push(new Molecule(0.05+counter*0.15, 0.4, tmp2, 1))
	    counter += 1;
	}
	counter = 0;

	for (var k in puzzle.products) {
	    atoms.push(new Molecule(0.63+counter*0.15, 0.07, puzzle.products[k], 2))
	    counter += 1;
	}
