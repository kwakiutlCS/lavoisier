$(function() {
	...
	for (var i = 0; i < 4; i++) {
		...
		if (Modernizr.localstorage) {
		    if (localStorage[t]) {
			ctx.drawImage(right, dx+(0.15+0.12*j)*w, dy+(0.185+0.2*i)*h, w*0.04, h*0.06);
		    }
		}
	    }
	} 
	...
	if (collection[menu_selection].reproduction === 2) {
	    locked = true;
	}
	...
	while (possible.length) {
	    map.push(possible.splice(parseInt(Math.random()*possible.length),1)[0]);
	}
	
	var output = {reagents:[], start:p.start, products:[]};

	for (var mol in p.reagents) {
	    console.log(output.reagents);
	    output.reagents.push({});
	    for (var i in p.reagents[mol]) {
		output.reagents[mol][map[i]] = p.reagents[mol][i];
	    }
	}
	for (var mol in p.products) {
	    output.products.push({});
	    for (var i in p.products[mol]) {
		output.products[mol][map[i]] = p.products[mol][i];
	    }
	}
	...
	for (var k in puzzle.reagents) {
	    ...
	    for (var i = 0; i<puzzle.start[index]; i++) {
